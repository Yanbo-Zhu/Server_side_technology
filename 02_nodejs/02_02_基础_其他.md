...
Beispiel:
```
const jsonString = '{"name": "Szymon", "age": 26}';
...
Beispiel:
```js
const myObject = { name: "Szymon", age: 26 };
...
1. Erstellt einen Server, der eine Callback-Funktion erhält.
...
2. Reagiert auf eingehende Anfragen (Response-Methoden, Daten-Stream-Handler)
3. Startet den Server mit .listen(PORT, callback)
...
4. Standardmäßig ist http.createServer() für kleine Projekte geeignet, für größere Projekte ist Express.js besser.
...
text/xxx
1. Dieser Typ wird verwendet, wenn der Inhalt aus lesbarem Text besteht, den Menschen direkt verstehen und interpretieren können.
...
application/xxx
1. Dieser Typ wird verwendet, wenn die Daten nicht direkt als Text interpretiert werden können oder wenn sie eine spezifische Anwendung verarbeiten soll.
2. Typische Eigenschaften:
